...
//import function to mock the api call
...
test('App renders without errors', async()=>{
    ...
    expect(screen.queryByText(/love letter/i)).toBeNull();
    expect(screen.queryByTestId(/dropdown/i)).toBeNull();
    
    //wait for promise to finish
    await waitFor(async() => {
        expect(await screen.findByText(/love letter/i));
    })

    //assert that it's rendered to the screen
    expect(screen.getByText(/love letter/i)).toBeInTheDocument();
